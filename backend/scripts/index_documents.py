...
def index_docs():
    '''
    Reads PDFs, Chunks, uploads to Azure AI Search
    '''
    current_dir = os.path.dirname(os.path.abspath(__file__))
    data_folder = os.path.join(current_dir,"backend\data")

    logger.info("="*60)
    logger.info("Environement Configuration Check:")
    logger.info(f"AZURE_OPENAI_ENDPOINT : {os.getenv('AZURE_OPENAI_ENDPOINT')}")
    logger.info(f"AZURE_OPENAI_API_VERSION : {os.getenv('AZURE_OPENAI_API_VERSION')}")
    logger.info(f"Embedding Doployment : {os.getenv('AZURE_OPENAI_EMBEDDING_DEPLOYMENT', "text-embedding-3-small")}")
    ...
    if missing_vars:
        logger.error(f"Missing required environment varibles : {missing_vars}")
    ...
    try:
        ...
        logger.info("Vector Store Initialized for the index: {index_name}")
    ...
    for pdf_path in pdf_files:
        try:
            ...
            logger.info(f"Split into {len(splits)} splits.")
